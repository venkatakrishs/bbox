...
'''
File Description: This file consists cocotb testbench for bbox dut

TODO:
Task Description: Add list of instructions. So that testbench generates tests for listed instructions. One instruction is implemented as an example. For multiple instructions, provided                  as comment (see after TestFactory(TB)). Please the use the same format.
                  Note - Comments are provided for TestFactory.

Note - Here testbench assumes below port names are generated by bluespec compiler. Please implement the design with below port names.
